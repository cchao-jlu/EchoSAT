...
def sample_random_var_params(
    ...
    """
        Attach random variable parameterizations to PyG graphs without running the GNN.
        This is used to create solver warmup states for one-step refinement training.
    """
    data_list_all = []
...
def sample_var_params(
    ...
    for i, data in enumerate(loader):
        ...
        if cache is not None:
            base_embedding = unbatch(cache["base_embedding"].to("cpu"), var_batch)
            base_y = unbatch(cache["base_y"].to("cpu"), var_batch)
        else:
            base_embedding = None
            base_y = None
        ...
        for j, data in enumerate(data_list):
            ...
            if base_embedding is not None and base_y is not None:
    ...
    def iter_inputs():
        for data in data_list:
            ...
            for j in range(var_params_all.shape[1]):
                cnf = dataset.get_cnf(cnf_id) if hasattr(dataset, "get_cnf") else dataset.cnf_list[cnf_id]
