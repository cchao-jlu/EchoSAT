...
SOLVER_BIN_PATHS = {
    ...
    "glucose_weighted": "solvers/glucose_weighted/simp/glucose_release",
...
def stdout_to_results_dict(stdout: str, collect_events: bool = False) -> dict[str, Any]:
    ...
    for line in stdout.splitlines():
        ...
        if collect_events and line.startswith(EVENT_PREFIX):
            ...
            if len(parts) >= 2:
                key = f"event_{parts[0]}"
                stats[key] = [float(value) for value in parts[1:]]
            continue
        for stat in STATS:
...
def solve_cnf(
    ...
    collect_events = bool(params.pop("collect-events", False))
    ...
    if solver != "march":
        ...
        timeout = params.get("cpu-lim", None)
        ...
        try:
            ...
            stats = stdout_to_results_dict(exc.stdout or "", collect_events=collect_events)
            stats["Result"] = "TIMEOUT"
            if timeout is not None:
                stats["CPU time"] = float(params.get("cpu-lim", 0.0))
            return stats
        finally:
            try:
    ...
    stats = stdout_to_results_dict(result.stdout, collect_events=collect_events)
    return stats
