...
class DimacsCNFDataset(Dataset):
    ...
    def __init__(
        ...
        files = list(glob(self.path))
        files.sort()
        if not files:
            raise ValueError(f"No DIMACS files found for path/pattern: {path}")
        self.id_to_file = {i: fn for i, fn in enumerate(files)}
    ...
    def get_cnf(self, idx: int) -> CNF:
        if not self.lazy:
            return self.cnf_list[idx]
        return CNF(from_file=self.id_to_file[int(idx)])
